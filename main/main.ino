...
void playProductAudio(String productLabel) {
  String fileName = productLabel + ".mp3";
  String filePath = String(PRODUCT_FOLDER) + "/" + fileName;

  Serial.printf("Trying to play product audio: %s\n", filePath.c_str());

  // Check if file exists
  if (!SD.exists(filePath)) {
    Serial.printf("Product audio file not found: %s\n", filePath.c_str());

    // Try alternative filename formats
    String altFileName1 = productLabel;
    altFileName1.replace(" ", "_");
    altFileName1 += ".mp3";
    String altFilePath1 = String(PRODUCT_FOLDER) + "/" + altFileName1;

    String altFileName2 = productLabel;
    altFileName2.replace(" ", "");
    altFileName2 += ".mp3";
    String altFilePath2 = String(PRODUCT_FOLDER) + "/" + altFileName2;

    if (SD.exists(altFilePath1)) {
      filePath = altFilePath1;
      Serial.printf("Found alternative file: %s\n", filePath.c_str());
    } else if (SD.exists(altFilePath2)) {
      filePath = altFilePath2;
      Serial.printf("Found alternative file: %s\n", filePath.c_str());
    } else {
      Serial.printf("No product audio file found for: %s\n", productLabel.c_str());
      // Return to IDLE state since product not found
      currentState = IDLE;
      digitalWrite(LED_PIN, LOW);
      lastActionTime = millis();
      robot.idle();
  ...
  Serial.printf("Playing product audio: %s\n", filePath.c_str());
  audio.stopSong();
  audioFinished = false;
  audio.connecttoFS(SD, filePath.c_str());
  ...
  if (!audio.isRunning()) {
    Serial.printf("Failed to play product audio: %s\n", filePath.c_str());
    ...
    // Set state to PRODUCT_DETECTED to handle audio finishing
    currentState = PRODUCT_DETECTED;
    digitalWrite(LED_PIN, HIGH);
    robot.answer();  // Perform answer action while playing audio
  }
}
...
void callback(char *topic, byte *message, unsigned int length) {
  ...
  if (String(topic) == "testtopic/mwtt") {
    ...
    // Handle "command" field for product detection
  ...
  if (currentState == PLAYING_RESPONSE || currentState == DANCE || currentState == HAPPY || currentState == MAD || currentState == SAD || currentState == GREETINGS || currentState == PRODUCT || currentState == PRODUCT_DETECTED || currentState == DETECTING_WAKE_WORD || currentState == TRANSITION || currentState == RECORDING_CONVERSATION || currentState == THINKING) {
    if (currentTime - lastAudioTime >= 1) {
      ...
      if (!audio.isRunning() || audioFinished) {
        // Handle audio finished for different states
        if (currentState == PRODUCT_DETECTED) {
          Serial.println("Product audio finished - returning to IDLE state");
          audioFinished = false;
          currentState = IDLE;
          digitalWrite(LED_PIN, LOW);
